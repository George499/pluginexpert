...
const ProfileEditForm = ({ 
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-white rounded-lg p-6 shadow-md text-black">
      <h2 className="text-xl font-semibold mb-4">{profileExists ? "Редактирование профиля" : "Создание профиля"}</h2>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4 mb-4">
          {galleryPreviews.map((preview, index) => (
            <div key={index} className="relative group">
              <div className="aspect-w-16 aspect-h-9 bg-gray-100 rounded-lg overflow-hidden" style={{height: '160px', width: '100%'}}>
                <img src={preview} alt={`Изображение ${index + 1}`} className="w-full h-full object-cover" style={{objectFit: 'cover'}} />
              </div>
              <button
                type="button"
          ...
          >
            {isCompressingGallery ? (
              ...
              <>
                ...
                <span className="mt-2 text-sm text-gray-600">Добавить фото</span>
              </>
            )}
          </div>
        </div>
      ...
      {/* Категории */}
      <div className="mb-6">
        <label className="block text-sm font-medium mb-3">Категории выступлений</label>
        <div className="border border-gray-300 rounded-md p-4 max-h-64 overflow-y-auto">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
            {availableCategories.sort((a, b) => (a.index || 0) - (b.index || 0)).map(category => (
      ...
      {/* Темы выступлений */}
      <div className="mb-6">
        <label className="block text-sm font-medium mb-1">Темы выступлений</label>
        <textarea
      ...
      <div className="mb-6">
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          ...
          <div>
            <label className="block text-sm font-medium mb-1">Одноклассники</label>
            <input
              type="text"
              value={formData.ok}
