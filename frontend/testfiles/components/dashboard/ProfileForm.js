...
const ProfileForm = () => {
    ...
    try {
      ...
      if (userData.speaker) {
        ...
        if (userData.speaker.avatar) {
          ...
          else if (userData.speaker.avatar.data && userData.speaker.avatar.data.url) {
        ...
        if (userData.speaker.gallery && userData.speaker.gallery.data && Array.isArray(userData.speaker.gallery.data)) {
        ...
        else if (userData.speaker.gallery && Array.isArray(userData.speaker.gallery)) {
  ...
  useEffect(() => {
    const fetchUserProfile = async () => {
      try {
        ...
        const res = await fetch(`${API_URL}/api/users/me?populate[speaker][populate][0]=avatar&populate[speaker][populate][1]=gallery`, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        });
        ...
        if (userData.speaker) {
          ...
          // Загрузка галереи изображений
          if (userData.speaker.gallery && userData.speaker.gallery.data && Array.isArray(userData.speaker.gallery.data)) {
            const galleryUrls = userData.speaker.gallery.data.map(item => {
            ...
            const galleryUrls = userData.speaker.gallery.map(item => {
              if (item.url) {
  ...
  const handleAvatarChange = async (e) => {
    ...
    try {
      ...
      if (file.size > MAX_SIZE) {
        ...
        toast(`Файл слишком большой (${(file.size / (1024 * 1024)).toFixed(2)}MB). Автоматическое сжатие...`);
        ...
        if (compressedFile.size > MAX_SIZE) {
          toast.error(`Не удалось сжать изображение до требуемого размера. Пожалуйста, выберите другой файл.`);
          setIsCompressingAvatar(false);
          return;
        }
        
        toast.success(`Изображение успешно сжато до ${(compressedFile.size / (1024 * 1024)).toFixed(2)}MB`);
      ...
      for (const file of files) {
        ...
        if (file.size > MAX_SIZE) {
          try {
            ...
            // Проверяем, успешно ли сжалось изображение
            if (compressedFile.size > MAX_SIZE) {
              toast.error(`Не удалось сжать изображение ${file.name} до требуемого размера.`);
              continue;
            }
      ...
      const updateGalleryState = (images, previews) => {
        ...
        setTimeout(() => {
          ...
          if (compressedCount > 0) {
            toast.success(`${compressedCount} изображений были автоматически сжаты`);
          }
  ...
  const uploadAvatar = async (token, profile) => {
    ...
    if (avatar.size > MAX_SIZE) {
      toast.error(`Файл слишком большой! Максимальный размер: 5MB. Текущий: ${(avatar.size / (1024 * 1024)).toFixed(2)}MB`);
      return false;
    }
    ...
    try {
      ...
      if (!uploadRes.ok) {
        ...
        try {
          ...
          console.error("Ошибка загрузки аватара:", uploadRes.status, errorData);
          errorMessage = `Ошибка загрузки аватара: ${errorData.error?.message || uploadRes.status}`;
        } catch (e) {
          const errorText = await uploadRes.text();
          console.error("Ошибка загрузки аватара:", uploadRes.status, errorText);
        }
      ...
      // Если результат успешный и есть URL загруженного файла
      ...
      toast.error(`Ошибка загрузки аватара: ${error.message || "Неизвестная ошибка"}`);
      return false;
    }
  };
  ...
  const prepareGalleryData = async () => {
    ...
    try {
        ...
        if (previewUrl.startsWith('data:')) {
          ...
          const promise = (async () => {
            try {
              ...
              console.error(`Ошибка обработки изображения ${previewUrl}:`, error);
      ...
      console.log(`Подготовлено ${allFiles.length} файлов для сохранения`, allFiles);
      
      toast.success(`Подготовлено ${allFiles.length} изображений для сохранения`);
      return allFiles;
    } catch (error) {
  ...
  const updateGallery = async (token, profile, galleryFiles) => {
    if (!profile || !profile.id) {
      console.error("Отсутствует необходимый ID профиля для обновления галереи");
      return false;
    }
    
    try {
      // Сначала удаляем все связи с текущими изображениями в галерее
      const updateRelationUrl = `${API_URL}/api/speakers/${profile.documentId || profile.id}`;
      ...
      if (galleryFiles && galleryFiles.length > 0) {
        ...
        if (!uploadRes.ok) {
          ...
          try {
            const errorData = await uploadRes.json();
            errorMessage = `Ошибка загрузки галереи: ${errorData.error?.message || uploadRes.status}`;
          } catch (e) {
            const errorText = await uploadRes.text();
            console.error("Ошибка загрузки галереи:", uploadRes.status, errorText);
          }
        ...
        if (Array.isArray(result) && result.length > 0) {
          toast.success(`Галерея успешно обновлена (загружено ${result.length} файлов)`);
        }
      } else {
      ...
      toast.error(`Ошибка обновления галереи: ${error.message || "Неизвестная ошибка"}`);
      return false;
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (profileExists && profile) {
        ...
        if (!documentId) {
          console.error("Не удалось определить documentId профиля для обновления");
        ...
        try {
          ...
          if (updateRes.ok) {
            ...
            try {
              ...
              errorMessage = `Ошибка обновления профиля: ${errorData.error?.message || "Неизвестная ошибка"}`;
            } catch (e) {
              const errorText = await updateRes.text();
              console.error("Ошибка обновления профиля:", errorText);
              errorMessage = `Ошибка обновления профиля: ${errorText || updateRes.status}`;
            }
          ...
          toast.error(`Ошибка обновления профиля: ${updateError.message || "Неизвестная ошибка"}`);
          setProfile({ ...profile, ...speakerData });
        }
      }
        ...
        try {
          ...
          if (createRes.ok) {
            ...
            if (result.data) {
              ...
              newDocumentId = result.data.attributes?.documentId || result.data.documentId;
            } else {
            ...
            try {
              const updateUserRes = await fetch(`${API_URL}/api/users/${userId}`, {
                method: "PUT",
                headers: {
                  Authorization: `Bearer ${token}`,
                  "Content-Type": "application/json",
              ...
              console.error("Ошибка связывания профиля с пользователем:", userUpdateError);
            }
            ...
            try {
              ...
              errorMessage = `Ошибка создания профиля: ${errorData.error?.message || "Неизвестная ошибка"}`;
            } catch (e) {
              const errorText = await createRes.text();
              console.error("Ошибка создания профиля (текст):", errorText);
              errorMessage = `Ошибка создания профиля: ${errorText || createRes.status}`;
            }
